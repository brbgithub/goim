...
func handleMessage(msg *nsq.Message) error {
	...
	// 发送消息
	err = ctx.Codec.Encode(Package{Code:CodeMessage,Content:content},WriteDeadline)
	if err != nil {
		logger.Sugar.Error(err)
		return err
	}
...
func handleMessageSendACK(msg *nsq.Message) error {
	...
	// 发送消息
	err = ctx.Codec.Encode(Package{Code: CodeMessageSendACK, Content: content}, WriteDeadline)
	if err != nil {
		logger.Sugar.Error(err)
		return err
	}
